...
Параметры пресетов зависят от используемых в DX7 микросхем и поэтому выглядят ужасно. Например так выглядит описание параметров ADSR-огибающей
...
Самое запутанное — это то, что регуляторы скорости инвертированы, поэтому на DX7 для увеличения времени огибающей необходимо опустить ползунок. Кроме того, уровень затухания также устанавливает начальный уровень. 
```
...
type SynthPreset = {
	...
	operators: { 
		...
		volume: number;
		envelope: { 
				attack: { duration: number; values: number[]; };
				decay: { duration: number; values: number[]; };
				sustain: { duration: number; values: number[]; };
				release: number; }; 
			}[];
